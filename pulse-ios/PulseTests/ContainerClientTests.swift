/// Unit tests for `PulseClient` container-related endpoints.
...
final class ContainerClientTests: XCTestCase {
    ...
    /// Builds a `PulseClient` pointed at the stub URL with a fixed bearer.
    /// Inputs:
    ///   - responder: closure that returns a stubbed HTTP response.
    /// Outputs: a `PulseClient` ready to make stubbed requests.
    ...
    /// Verifies an HTTP 413 response maps to `PulseError.payloadTooLarge`.
