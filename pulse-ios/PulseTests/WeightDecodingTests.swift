...
/// Confirms weights round-trip through `pulseDefault()` with their
...
final class WeightDecodingTests: XCTestCase {
    ...
    func testDecodeWeightEntries() throws {
        ...
        let entries = try JSONDecoder.pulseDefault().decode([WeightEntry].self, from: data)
    ...
    func testDecodeCaloriesDaily() throws {
        ...
        let rows = try JSONDecoder.pulseDefault().decode([CaloriesDailyRow].self, from: data)
