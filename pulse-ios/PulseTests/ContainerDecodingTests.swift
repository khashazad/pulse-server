...
/// Confirms the shared `pulseDefault()` decoder maps the server's
...
final class ContainerDecodingTests: XCTestCase {
    ...
    func testDecodeContainersList() throws {
        ...
        let list = try JSONDecoder.pulseDefault().decode(ContainersList.self, from: data)
    ...
    func testDecodeSingleContainer() throws {
        ...
        let c = try JSONDecoder.pulseDefault().decode(Container.self, from: data)
