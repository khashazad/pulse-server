...
When asked to "open Xcode" (or similar), run `source .envrc && xcodegen generate && open DietTracker.xcodeproj` — never open the project without sourcing `.envrc` first, or builds will fail the prebuild URL check.
