...
/// Mirrors the auth + error-mapping conventions of `PulseClient`.
...
actor ProgressPhotoClient {
    ...
    private func makeURL(path: String, query: [URLQueryItem]) throws -> URL {
        ...
        ) else { throw PulseError.notSignedIn }
    ...
    /// Transport failures surface as `PulseError.network`; a reachable
    /// but non-HTTP response surfaces as `PulseError.server(status: -1)`
    ...
    private func mapStatus(_ status: Int) throws {
        ...
        case 409:      throw PulseError.server(status: 409)
